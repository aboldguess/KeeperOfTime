...
def create_dummy_project():
    ...
    project = Project(name=project_name, description='Autogenerated dummy project')
    db.session.add(project)
    db.session.flush()  # Ensure project.id is available

    # Create three work packages each with three tasks
    for i in range(1, 4):
        wp = WorkPackage(name=f'Work package #{i}', project_id=project.id)
        db.session.add(wp)
        db.session.flush()
        for j in range(1, 4):
            task = Task(name=f'Task #{j}', work_package_id=wp.id)
            db.session.add(task)
